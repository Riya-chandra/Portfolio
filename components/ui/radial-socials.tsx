...
const RadialIcon = React.forwardRef<HTMLDivElement, InternalRadialIconProps>(
	(
		...
		const { animatedIcons, expandDuration, calculatePosition, rotationStarted } =
			useRadialSocials();
				...
				>
					{React.Children.map(children, (child, iconIndex) => {
						if (
							...
							// compute angle per icon respecting startAngle so icons are placed correctly
							const step = icons.length > 0 ? 360 / icons.length : 360;
							const computedAngle = startAngle + step * (iconIndex as number);
...
const RadialSocials = React.forwardRef<HTMLDivElement, RadialSocialsProps>(
	({ children, className, animationDelay = 150, expandDuration = 800, ...props }, ref) => {
		...
		useEffect(() => {
			// use a ref to hold timeout ids so we can clear them on unmount or when dependencies change
			...
			return () => {
				...
				// reset state when unmounted or deps change so animations can replay
				setAnimatedIcons(new Set());
